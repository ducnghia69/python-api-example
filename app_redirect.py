    ...
    def redirect(options):
        ...
        if has_ios and ("iPhone" in user_agent or "iPad" in user_agent or "iPod" in user_agent):
            ...
            intent_url = f'intent://{intent["host"]}#Intent;' \
                    ...
                    f'{"action=" + intent["action"] + ";" if intent.get("action") else ""}' \
                    f'{"category=" + intent["category"] + ";" if intent.get("category") else ""}' \
                    f'{"component=" + intent["component"] + ";" if intent.get("component") else ""}' \
                    f'{"S.browser_fallback_url=" + intent["fallback"] + ";" if intent.get("fallback") else ""}' \
                    'end'
